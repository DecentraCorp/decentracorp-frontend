...
export const knownDIDsAtom = atom(
  getKnownDIDs(),
  (_get, set, dids: KnownDIDs) => {
    setKnownDIDs(dids);
...
export function getInitialEnv(checkLocal = true): EnvState {
  const client = new WebClient({
    ceramic: CERAMIC_URL,
    connectNetwork: CONNECT_NETWORK as any,
  });
  if (!checkLocal) {
...
export const authDIDDataAtom = atom(
  ...
  (get, set, update: Partial<KnownDIDData>) => {
    ...
    const { auth } = get(envStateAtom);
    if (auth.id != null) {
      set(knownDIDsDataAtom, {
        ...didsData,
        [auth.id]: { ...didsData[auth.id], ...update },
      });
    }
  },
);
...
export type EditProfileState = {
  status: 'pending' | 'editing' | 'failed' | 'done';
  error?: Error;
};
